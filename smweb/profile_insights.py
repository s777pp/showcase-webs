...
Every search query must be a concise ENGLISH keyword phrase even when the rest of the JSON is Russian. It must preserve the concrete subject and end with "Steam artwork showcase". Provide close semantic alternatives rather than unrelated generic moods. Never suggest Google Images or broad wallpaper queries.
...
def generate(kind: str, profile: dict, language: str = "en", style: str = "auto") -> dict:
    ...
    task = (
        ...
        if kind == "doctor" else
        "Return summary and exactly 3 compact concepts. First inspect the supplied showcase visuals and infer their dominant subject, medium and genre; if uncertain, say so in the summary but keep the most likely visible subject in all concepts. Each concept: title, precise style (subject + medium + genre, not a vague mood), 3-5 hex colors in palette, a concrete Steam showcase_prompt naming subject, scene, composition, motion idea and showcase type, 3-4 search_queries, and why_it_fits tied to observed profile elements. Search queries are for DeviantArt: write them in English, preserve the exact subject, add one close stylistic alternative, and end each with 'Steam artwork showcase'. Keep summary under 300 characters; showcase_prompt and why_it_fits under 500 characters each; every search query under 100 characters. Do not add prose outside these fields."
    )
