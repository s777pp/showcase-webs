(function () {
  ...
  function renderCanvas(now) {
    ...
    project.layers.forEach(function(layer){if(layer.visible===false||(diagnostic&&layer.type==='background'))return;if(layer.type==='text')drawText(layer,now);else if(layer.type==='frame')drawFrame(layer);else if(layer.type==='effect')drawEffect(layer,now);else drawMediaLayer(layer,mediaFor(layer),now)});
  }
  function draw(now) {
  ...
  function renderLayers(){
    ...
    project.layers.slice().reverse().forEach(function(layer){
      ...
      row.querySelector('.builder-layer__copy b').textContent=layer.name;row.querySelector('.builder-layer__copy span').textContent=t(layer.type);
  ...
  function projectAnimated(){return project.layers.some(function(l){return l.visible!==false&&(l.type==='effect'||l.animation&&l.animation!=='none'||l.mediaType==='image/gif'||l.mediaType&&l.mediaType.indexOf('video/')===0||/\.gif(\?|$)/i.test(l.src||''))})}
