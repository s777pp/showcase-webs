(function () {
  ...
  Object.keys(NEW_COPY).forEach(function(language){COPY[language]=Object.assign({},COPY[language]||{},NEW_COPY[language],VFX_COPY[language]||{},GRADE_COPY[language]||{})});
  ...
  function renderLayers(){
    ...
    project.layers.slice().reverse().forEach(function(layer){
      ...
      row.onclick=function(){selected=layer.id;renderLayers();syncInspector()};row.onkeydown=function(e){if(e.target===row&&(e.key==='Enter'||e.key===' ')){e.preventDefault();row.click()}};host.appendChild(row);
    });
  ...
  syncInspector=function(){syncInspectorBase();if(motionEngine)motionEngine.sync();window.WorkspaceEditor?.builderState({count:project.layers.length,selected:!!currentLayer(),exportable:project.layers.some(function(layer){return layer.visible!==false})})};
  ...
  function projectAnimated(){return !!motionEngine?.hasAnimation()||project.layers.some(function(l){return l.visible!==false&&(l.type==='effect'||l.type==='frame'&&!!window.SMSquaresFx?.isAnimatedFrame(l.frameStyle)||l.type==='dna'||l.animation&&l.animation!=='none'||l.mediaType==='image/gif'||l.mediaType&&l.mediaType.indexOf('video/')===0||/\.gif(\?|$)/i.test(l.src||''))})}
  ...
  function applyLanguage(){applyFrameFxLanguage();document.querySelectorAll('[data-builder-i]').forEach(function(n){n.textContent=t(n.dataset.builderI)});document.querySelectorAll('[data-builder-i-title]').forEach(function(n){var value=t(n.dataset.builderITitle);n.title=value;n.setAttribute('aria-label',value)});renderLayers();loadProjects()}
