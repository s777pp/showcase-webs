...
function appendCatalog(items,q){var host=$('catalog');(items||[]).filter(function(it){return !q||String(it.name||it.game||'').toLowerCase().indexOf(q.toLowerCase())>=0}).forEach(function(it){var u=it.movie||it.video||it.image||it.url||'';if(!u)return;var key=it.defid||it.market_url||it.buy_url||u;if(host.querySelector('[data-asset-key="'+CSS.escape(String(key))+'"]'))return;var poster=it.preview||it.image||u,b=document.createElement('button');b.type='button';b.className='asset';b.dataset.assetKey=String(key);b.innerHTML=/\.(webm|mp4)(\?|$)/i.test(u)?'<video src="'+SteamMockup.esc(SteamMockup.px(u))+'" poster="'+SteamMockup.esc(SteamMockup.px(poster))+'" muted autoplay loop playsinline></video>':'<img src="'+SteamMockup.esc(SteamMockup.px(poster))+'" alt=""><span>'+SteamMockup.esc(it.name||it.game||'Steam')+'</span>';b.onclick=function(){selectCatalogItem(it,u,poster,b)};host.appendChild(b)})}
function extensionCatalog(q,kind){var types;if(asset==='background')types=kind==='animated'?['animated_background']:(kind==='static'?['background']:['animated_background','background']);else if(asset==='frame'||asset==='avatar')types=[asset];else return Promise.resolve();return extMessage({type:'PING'}).then(function(){return Promise.all(types.map(function(t){return extMessage({type:'GET_CUSTOMIZATION_CATALOG',asset:t,page:page,count:30})}))}).then(function(parts){parts.forEach(function(d){if(d&&d.ok)appendCatalog(d.items||[],q)})})}
function serverCatalog(q,kind){var requests=[];if(asset==='background'){if(kind!=='animated')requests.push(['background','static'],['points_background','all']);if(kind!=='static')requests.push(['animated_background','animated'])}else requests.push([asset,kind]);return Promise.all(requests.map(function(pair){return fetch('/api/steam/backgrounds?asset='+pair[0]+'&kind='+pair[1]+'&q='+encodeURIComponent(q)+'&page='+page+'&count=30').then(function(r){return r.json()})})).then(function(parts){var ok=false,msg='Catalog unavailable';parts.forEach(function(d){if(d&&d.ok){ok=true;appendCatalog(d.items||[],q)}else if(d&&d.msg)msg=d.msg});if(!ok)throw Error(msg);page++;return true})}
function loadAssets(reset){if(reset){page=0;$('catalog').innerHTML='';var buy=ensureAssetBuy();buy.hidden=true}var q=$('assetSearch').value.trim(),kind=$('assetKind').value;serverCatalog(q,kind).then(function(){if(reset)extensionCatalog(q,kind).catch(function(){})}).catch(function(e){extensionCatalog(q,kind).catch(function(){setStatus('saveState',e.message,'bad')})})}
