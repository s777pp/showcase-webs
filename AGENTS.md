...
- Workshop GIF output is a synchronized five-file group, not five independent conversions. `process_gif_workshop()` decodes and scales the source once, derives all panel frames from that shared timeline, and `_encode_workshop_frame_group()` selects the highest one quality setting that keeps every panel within 5 MiB. Never run per-panel FPS, frame dropping, scaling, or quality fitting; a lower common setting is preferable to visible seams or timeline drift.
