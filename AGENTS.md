...
- Design Selection reference actions are deliberately DeviantArt-only buttons generated from Gemini's subject-aware English search keywords. Queries must remain broad (maximum three words) because long literal phrases such as `monochrome ink dragon Steam artwork showcase` produce empty DeviantArt results. The intended four-query cascade is subject, subject+medium, genre+subject, and subject+Steam. Explanatory AI output still follows the RU/EN interface language. Do not restore generic Google Images or unrelated wallpaper links. Third-party reference media is not stored by the service.
