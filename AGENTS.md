...
- Design Selection reference actions are deliberately DeviantArt-only buttons generated from Gemini's subject-aware English search keywords and constrained to Steam artwork showcases. Explanatory AI output still follows the RU/EN interface language. Do not restore generic Google Images or unrelated wallpaper links. Third-party reference media is not stored by the service.
