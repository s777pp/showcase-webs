...
def test_extension_guide_is_localized_and_public():
    assert client.get('/extension', follow_redirects=False).headers['location'] in ('/en/extension', '/ru/extension')
    for language in ('ru', 'en'):
        response = client.get(f'/{language}/extension')
        assert response.status_code == 200
